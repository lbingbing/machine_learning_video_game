...
def create_state():
    return gridwalk_state.create_state()
...
def draw_canvas(state, unit_size, painter):
    for i in range(state.get_canvas_shape()[0]):
        for j in range(state.get_canvas_shape()[1]):
            ...
            if state.canvas[i][j] == gridwalk_state.BACKGROUND:
                painter.setBrush(QtGui.QColor(255, 255, 255))
            elif state.canvas[i][j] == gridwalk_state.TARGET:
                painter.setBrush(QtGui.QColor(0, 0, 255))
            elif state.canvas[i][j] == gridwalk_state.WALKER:
