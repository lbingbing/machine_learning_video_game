...
class FlappyBirdSGNSarsaTorchNNModel(sgnsarsa_torch_nn_model.SGNSarsaTorchNNModel):
    def create_network(self, state):
        return Network(state.get_state_numpy_shape(), state.get_action_dim())

class FlappyBirdSGNSarsaTorchNNPlayer(model_player.ModelPlayer):
    def create_model(self, state):
        return FlappyBirdSGNSarsaTorchNNModel(state)
