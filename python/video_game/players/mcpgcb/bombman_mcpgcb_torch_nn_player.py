...
class BombManMCPGCBTorchNNModel(mcpgcb_torch_nn_model.MCPGCBTorchNNModel):
    def create_network(self, state):
        return Network(state.get_state_numpy_shape(), state.get_action_dim())

class BombManMCPGCBTorchNNPlayer(model_player.ModelPlayer):
    def create_model(self, state):
        return BombManMCPGCBTorchNNModel(state)
