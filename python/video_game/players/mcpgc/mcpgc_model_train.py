...
def main(state, model, configs):
    ...
    for iteration_id in itertools.count(1):
        ...
        if need_check:
            ...
            scores.clear()
            ages.clear()
            print('iteration: {} avg_loss: {:.8f} P_logit_range: [{:.4f}, {:.4f}] P_range: [{:.4f}, {:.4f}] avg_score: {:.2f} avg_age: {:.2f}'.format(iteration_id, avg_loss, *P_logit_range, *P_range, avg_score, avg_age))
