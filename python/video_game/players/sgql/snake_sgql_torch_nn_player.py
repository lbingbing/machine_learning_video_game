...
class SnakeSGQLTorchNNModel(sgql_torch_nn_model.SGQLTorchNNModel):
    def create_network(self, state):
        return Network(state.get_state_numpy_shape(), state.get_action_dim())

class SnakeSGQLTorchNNPlayer(model_player.ModelPlayer):
    def create_model(self, state):
        return SnakeSGQLTorchNNModel(state)
