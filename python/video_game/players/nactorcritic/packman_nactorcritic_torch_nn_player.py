...
def create_model(state):
    return nactorcritic_torch_nn_model.NActorCriticTorchNNModel(state.get_name(), packman_torch_network.PVNetwork(state.get_state_numpy_shape(), state.get_action_dim()))
