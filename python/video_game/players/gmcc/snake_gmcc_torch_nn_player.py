...
class SnakeGMCCTorchNNModel(gmcc_torch_nn_model.GMCCTorchNNModel):
    def create_network(self, state):
        return Network(state.get_state_numpy_shape(), state.get_action_dim())

class SnakeGMCCTorchNNPlayer(model_player.ModelPlayer):
    def create_model(self, state):
        return SnakeGMCCTorchNNModel(state)
