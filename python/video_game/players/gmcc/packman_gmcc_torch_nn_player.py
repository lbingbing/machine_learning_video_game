...
class PackManGMCCTorchNNModel(gmcc_torch_nn_model.GMCCTorchNNModel):
    def create_network(self, state):
        return Network(state.get_state_numpy_shape(), state.get_action_dim())

class PackManGMCCTorchNNPlayer(model_player.ModelPlayer):
    def create_model(self, state):
        return PackManGMCCTorchNNModel(state)
